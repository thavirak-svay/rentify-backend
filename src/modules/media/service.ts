...
import { DatabaseError, ForbiddenError, NotFoundError, sanitizeFileName } from '@/shared/lib';
...
export async function deleteMedia(supabaseAdmin: SupabaseClient, userId: string, mediaId: string): Promise<void> {
  ...
  const { error: deleteError } = await supabaseAdmin.from('listing_media').delete().eq('id', mediaId);
  ...
  const urlParts = media.url.split('/listing-media/');
