...
function buildPricingInput(listing: ListingWithOwner, input: CreateBookingInput): PricingInput {
  ...
  return {
    ...
    deliveryFee:
      listing.delivery_available && input.delivery_method === DELIVERY_METHOD.DELIVERY ? listing.delivery_fee ?? 0 : 0,
...
export async function approveBooking(
  ...
  if (updateError) {
    log.warn(
      ...
      "DB update failed after payment capture - queuing compensation",
    )
    await queueCompensation(
      supabaseAdmin,
      "cancel_preauth",
      { payway_tran_id: transaction.payway_tran_id },
      bookingId,
      transaction.id,
    )
  ...
  if (txError) {
    log.error(
      { txError, bookingId, tran_id: transaction.payway_tran_id },
      "Transaction status update failed after approval",
    )
  }
...
export async function declineBooking(
  ...
  if (transaction?.id) {
    ...
    if (txError) {
      log.error(
        { txError, bookingId, tran_id: transaction.payway_tran_id },
        "Transaction status update failed after decline",
      )
    }
  }
