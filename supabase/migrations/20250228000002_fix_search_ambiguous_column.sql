-- Fix ambiguous column reference in search_listings function
-- The subquery needs to qualify thumbnail_url with the table alias
 ...
 BEGIN
   ...
   SELECT
     ...
     CASE
       ...
       ELSE NULL
     END AS distance_km,
     (SELECT lm.thumbnail_url FROM listing_media lm WHERE lm.listing_id = l.id ORDER BY lm.sort_order LIMIT 1),
     l.created_at
   FROM listings l
